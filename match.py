...
log_path = f"outputs/output-{datetime.today().strftime('%Y%m%d')}.json"   #Tiene que apuntar a MELI-scraper/outputs/output.json
...
def log_to_db(log_path, db):
    ...
    if os.path.isfile(match_path) and os.access(match_path, os.R_OK):
        ...
        with io.open(match_path, 'w') as db_file:
            ...
            {"seller_id": 0, "match": {"data_empresas": None}, 'matched': False}
            ))
            db_file.write('\n')
            ...
            for line in tail(log_file):
                ...
                if log_data['seller_id']:
                    ...
                    if log_data['seller_id'] not in VISITED_MATCHED:
                        ...
                        MATCHED = any([match_data['match'][k] for k in ['data_empresas']])
